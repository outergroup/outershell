...
int main(int argc, char* argv[]) {
    ...
    const char* backendIdentifier = nullptr;
    const char* displayName = nullptr;
    const char* plistPath = nullptr;
    const char* systemdUnitName = nullptr;
    const char* logPath = nullptr;
    const char* url = nullptr;
    const char* iconPath = nullptr;
    const char* frontendList = nullptr;
    const char* socketPath = nullptr;
    int port = 0;
    bool hasPort = false;
    bool ownsPlist = false;
    bool includeIconValues = false;

    for (int i = 3; i < argc; i += 1) {
        const char* argument = argv[i];
        auto requireValue = [&](const char* name, const char*& target) -> bool {
            if (i + 1 >= argc) {
        ...
        if (strcmp(argument, "--backend") == 0) {
            if (!requireValue("--backend", backendIdentifier)) {
                return 1;
            }
        } else if (strcmp(argument, "--port") == 0) {
            const char* rawPort = nullptr;
            if (!requireValue("--port", rawPort)) {
                return 1;
            }
            if (!parseInt(rawPort, port) || port <= 0 || port > 65535) {
                fprintf(stderr, "Invalid port: %s\n", rawPort);
                return 1;
            }
            hasPort = true;
        } else if (strcmp(argument, "--socket-path") == 0) {
            if (!requireValue("--socket-path", socketPath)) {
                return 1;
            }
        } else if (strcmp(argument, "--name") == 0) {
            if (!requireValue("--name", displayName)) {
                return 1;
            }
        } else if (strcmp(argument, "--plist") == 0) {
            if (!requireValue("--plist", plistPath)) {
                return 1;
            }
        } else if (strcmp(argument, "--unit") == 0) {
            if (!requireValue("--unit", systemdUnitName)) {
                return 1;
            }
        } else if (strcmp(argument, "--path") == 0) {
            if (!requireValue("--path", logPath)) {
                return 1;
            }
        } else if (strcmp(argument, "--owns-plist") == 0) {
            const char* rawValue = nullptr;
            if (!requireValue("--owns-plist", rawValue)) {
                return 1;
            }
            if (!parseBool(rawValue, ownsPlist)) {
                fprintf(stderr, "Invalid boolean for --owns-plist: %s\n", rawValue);
                return 1;
            }
        } else if (strcmp(argument, "--url") == 0) {
            if (!requireValue("--url", url)) {
                return 1;
            }
        } else if (strcmp(argument, "--icon-path") == 0 || strcmp(argument, "--icon-file") == 0) {
            if (!requireValue(argument, iconPath)) {
                return 1;
            }
        } else if (strcmp(argument, "--list") == 0) {
            if (!requireValue("--list", frontendList)) {
                return 1;
            }
        } else if (strcmp(argument, "--icons") == 0) {
            includeIconValues = true;
        } else {
    ...
    const bool isListAction = strcmp(action, "list") == 0;
    if (!isListAction && (!backendIdentifier || backendIdentifier[0] == '\0')) {
        fprintf(stderr, "Missing backend identifier.\n");
        return 1;
    }
    Buffer errorMessage;
    initBuffer(errorMessage);

    bool shouldAcquireRegistryLock = !isListAction;
    if (isListAction) {
        Buffer binaryPath;
        Buffer sqlitePath;
        initBuffer(binaryPath);
        initBuffer(sqlitePath);
        if (buildOuterwebappsPath(kRegistryBinaryFileName, binaryPath) &&
            buildOuterwebappsPath(kRegistryDatabaseFileName, sqlitePath)) {
            struct stat binaryStat {};
            struct stat sqliteStat {};
            shouldAcquireRegistryLock = stat(binaryPath.data, &binaryStat) != 0 &&
                stat(sqlitePath.data, &sqliteStat) == 0 &&
                S_ISREG(sqliteStat.st_mode);
        }
        freeBuffer(binaryPath);
        freeBuffer(sqlitePath);
    }
    ...
    if (shouldAcquireRegistryLock && !lock.acquire(errorMessage)) {
        fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to acquire registry lock.");
        freeBuffer(errorMessage);
        return 1;
    }

    sqlite3* database = nullptr;
    if (!openRegistryDatabase(database, errorMessage)) {
        fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to open registry database.");
        freeBuffer(errorMessage);
        return 1;
    }

    auto persistAndClose = [&]() -> bool {
        if (!exportRegistryBinaryFromDatabase(database, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to export registry binary.");
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return false;
        }
    ...
    if (isListAction) {
        if (!printRegistryList(database, resource, backendIdentifier, includeIconValues, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to list registry rows.");
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }
    ...
    if (strcmp(resource, "app") == 0) {
        if (!requireRegisteredBackend(database, backendIdentifier, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Backend not registered.");
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }

        HostedAppArray hostedApps;
        initHostedAppArray(hostedApps);
        if (!loadHostedAppsFromDatabase(database, backendIdentifier, hostedApps, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to load hosted apps.");
            freeHostedAppArray(hostedApps);
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }

        const bool hasSocketPath = socketPath && socketPath[0] != '\0';
        if ((hasPort ? 1 : 0) + (hasSocketPath ? 1 : 0) > 1) {
            printUsage();
            freeHostedAppArray(hostedApps);
            sqlite3_close(database);
            freeBuffer(errorMessage);
        ...
        if (strcmp(action, "add") == 0) {
            if ((!hasPort && !hasSocketPath) || !displayName || displayName[0] == '\0') {
                printUsage();
                freeHostedAppArray(hostedApps);
                sqlite3_close(database);
                freeBuffer(errorMessage);
            ...
            removeHostedAppsMatching(hostedApps, port, socketPath);

            HostedAppEntry entry;
            initHostedAppEntry(entry);
            entry.port = port;
            if (!assignCString(entry.socketPath, hasSocketPath ? socketPath : "")) {
                freeHostedAppEntry(entry);
            ...
            if (iconPath && iconPath[0] != '\0') {
                ...
                if (strncmp(iconPath, "data:", 5) == 0 || stat(iconPath, &iconStat) != 0 || !S_ISREG(iconStat.st_mode)) {
                    fprintf(stderr, "Icon path must be a regular file: %s\n", iconPath);
                    freeHostedAppEntry(entry);
                    freeHostedAppArray(hostedApps);
                    sqlite3_close(database);
                    freeBuffer(errorMessage);
            ...
            if (!assignCString(entry.iconPath, iconPath ? iconPath : "")) {
                freeHostedAppEntry(entry);
                fprintf(stderr, "Out of memory.\n");
                freeHostedAppArray(hostedApps);
                sqlite3_close(database);
            ...
            if (!assignCString(entry.name, displayName) ||
                !assignHostedAppURL(entry.url, url, port, socketPath) ||
                !assignCString(entry.list, frontendList ? frontendList : "") ||
                !appendHostedAppMove(hostedApps, entry)) {
                freeHostedAppEntry(entry);
                fprintf(stderr, "Out of memory.\n");
                freeHostedAppArray(hostedApps);
                sqlite3_close(database);
        ...
        if (!replaceHostedAppsInDatabase(database, backendIdentifier, hostedApps, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to persist hosted apps.");
            freeHostedAppArray(hostedApps);
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }
        if (!verifyHostedAppsInDatabase(database, backendIdentifier, hostedApps.count, errorMessage)) {
            fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Hosted app verification failed.");
            freeHostedAppArray(hostedApps);
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        ...
        if (strcmp(action, "add") == 0) {
#if defined(__APPLE__)
            postHostedAppOpenedDistributedNotification(backendIdentifier, port);
#endif
        } else {
#if defined(__APPLE__)
            postServiceUIsInvalidatedDistributedNotification();
#endif
        }
        return 0;
    }
    ...
    if (strcmp(resource, "backend") == 0) {
        if (strcmp(action, "upsert") == 0) {
            if (!upsertBackendRegistryRecord(database,
                                            backendIdentifier,
                                            (displayName && displayName[0] != '\0') ? displayName : backendIdentifier,
                                            iconPath,
                                            errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to upsert backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            ...
            bool exists = false;
            if (!backendRegistryRecordExists(database, backendIdentifier, exists, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to inspect backend registry.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            if (!exists) {
                fprintf(stderr, "Backend not registered.\n");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            ...
            bool countsOk = true;
            if (registrySupportsLaunchdBackends()) {
                countsOk = countRegistryRows(database,
                                             "SELECT COUNT(*) FROM launchd_backends WHERE service_id = ?;",
                                             backendIdentifier,
                                             launchdCount,
                                             errorMessage);
            }
            if (countsOk && registrySupportsSystemdBackends()) {
                countsOk = countRegistryRows(database,
                                             "SELECT COUNT(*) FROM systemd_backends WHERE service_id = ?;",
                                             backendIdentifier,
                                             systemdCount,
                                             errorMessage);
            }
            if (countsOk) {
                countsOk = countRegistryRows(database,
                                             "SELECT COUNT(*) FROM log_files WHERE service_id = ?;",
                                             backendIdentifier,
                                             logCount,
                                             errorMessage) &&
                    countRegistryRows(database,
                                      "SELECT COUNT(*) FROM frontends WHERE service_id = ?;",
                                      backendIdentifier,
                                      frontendCount,
                                      errorMessage);
            ...
            if (!countsOk) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to inspect backend references.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            if (launchdCount > 0 || systemdCount > 0 || logCount > 0 || frontendCount > 0) {
                fprintf(stderr, "Backend still has service-manager, log, or app records. Clear those first.\n");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            if (!deleteBackendRegistryRecord(database, backendIdentifier, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to remove backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
    ...
    if (strcmp(resource, "systemd") == 0) {
        if (!registrySupportsSystemdBackends()) {
            fprintf(stderr, "systemd registry is unavailable on this platform.\n");
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }
        ...
        if (strcmp(action, "set") == 0) {
            if (!systemdUnitName || systemdUnitName[0] == '\0') {
                printUsage();
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            if (!upsertSystemdBackendRegistryRecord(database,
                                                    backendIdentifier,
                                                    systemdUnitName,
                                                    errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to set systemd backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
        } else if (strcmp(action, "clear") == 0) {
            if (!deleteSystemdBackendRegistryRecord(database, backendIdentifier, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to clear systemd backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
    ...
    if (strcmp(resource, "launchd") == 0) {
        if (!registrySupportsLaunchdBackends()) {
            fprintf(stderr, "launchd registry is unavailable on this platform.\n");
            sqlite3_close(database);
            freeBuffer(errorMessage);
            return 1;
        }
        ...
        if (strcmp(action, "set") == 0) {
            ...
            if (!upsertLaunchdBackendRegistryRecord(database,
                                                    backendIdentifier,
                                                    plistPath,
                                                    ownsPlist,
                                                    errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to set launchd backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
        } else if (strcmp(action, "clear") == 0) {
            if (!deleteLaunchdBackendRegistryRecord(database, backendIdentifier, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to clear launchd backend.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
    ...
    if (strcmp(resource, "log") == 0) {
        ...
        if (strcmp(action, "add") == 0) {
            ...
            if (!insertLogFileRegistryRecord(database, backendIdentifier, logPath, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to add log file.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
            ...
            if (!deleteLogFileRegistryRecord(database, backendIdentifier, logPath, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to remove log file.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
        } else if (strcmp(action, "clear") == 0) {
            if (!clearLogFileRegistryRecords(database, backendIdentifier, errorMessage)) {
                fprintf(stderr, "%s\n", errorMessage.data ? errorMessage.data : "Failed to clear log files.");
                sqlite3_close(database);
                freeBuffer(errorMessage);
                return 1;
            }
